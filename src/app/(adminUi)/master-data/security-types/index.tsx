...
import { useFetchSecurityTypeQuery, useDeleteSecurityTypeMutation } from "./securityTypeApiSlice";
...
import EditSecurityTypeDrawer from "./editSecurityTypeDrawer";
import PromptModal from "@/components/Modals/PromptModal";
import Table from "@/components/Table";
import { GpToast } from "../../../../components/Toast";
import { toast } from "react-toastify";
import CreateSecurityTypeDrawer from "./createSecurityTypeDrawer";
...
export default function SecurityType(): React.ReactElement {
  const [page, setPage] = useState<number>(1);
  const [searchTerm, setSearchTerm] = useState("");
  const { data, refetch, isFetching } = useFetchSecurityTypeQuery();
  const [deleteBank, { isLoading, isSuccess, isError, error }] = useDeleteSecurityTypeMutation();
  ...
  const entityNameSingular = "Security type";
  ...
  if(isSuccess){
    ...
    GpToast({ type: 'success', message: "Security-type deleted successfully",
    ...
    <div className="px-8 pt-6 pb-10">
      <div className="mb-4">
        <Breadcrumb aria-label="Default breadcrumb example">
          ...
          <Breadcrumb.Item>Security Types</Breadcrumb.Item>
        </Breadcrumb>
      </div>
      {/* <SuccessModal /> */}
              ...
              <div className="hidden pb-6 border-b space-x-6 md:flex ">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Search for security types"
                  />
                </div>
              </div>
            </div>
      ...
      <PromptModal
        headingText={""}
        bodyText={"Are you sure you want to delete this security-type?"}

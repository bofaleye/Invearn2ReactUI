...
import { useDeleteAssetPriceMutation } from "./assetsApiSlice";
...
export interface AssetPriceTableProps {
  data?: IAssetPrice[] | undefined | null;
...
export default function AssetPriceTable({
  ...
  IAssetPrice[] | null
  >(data ?? []);
  const [selectedBank, setSelectedBank] = useState<IAssetPrice | null>(null);
  ...
  const tableAction = (id: string) => {
    let tableIconArray: TableActionItem[] = [
      // {
      //   text: "View Asset Price",
      //   icon: <Eye className="mr-3" />,
      //   disabled: false,
      //   visible: true,
      //   action: () => push(`asset-price/${id}`),
      // },
      {
        text: "Edit Asset Price",
...
function DeleteModal({ assetPrice, setOpen, refetch, open }: DeleteModalProps) {
  ...
  return (
    <PromptModal
      ...
      isOpen={open}
      OnConfirm={() => handleDelete(assetPrice?.id ?? "")}
